...
class ChangeBIO extends React.Component {
    constructor(props){
        super(props);
        this.state = {
            showError: false,
        ...
        return(
            <View style={{flex: 1, backgroundColor: 'white'}}>
                ...
                <TouchableOpacity onPress={() => this.props.navigation.goBack()} style={{height: 5*height/100, width: 5*height/100, backgroundColor: 'white', borderRadius: 7, marginBottom: 1*height/100, justifyContent: 'center'}}>
                    ...
                    <TextInput value={this.state.bio} onFocus={() => this.setState({bio: ''})} onChangeText={(bio) => this.setState({bio: bio})}  numberOfLines={4} selectionColor='#B34D8C' style={{width: '100%', paddingLeft: 5*width/100, height: 15*height/100, borderRadius: 7, borderWidth: 2, alignSelf: 'center', marginBottom: 2*height/100, fontFamily: fontStyle.FONT_FAMILY_REGULAR, fontSize: RFValue(10, width)}} placeholderTextColor='black' placeholder='Update BIO' />
                    <Text style={{backgroundColor: 'white', position: 'absolute', marginLeft: 10*width/100, marginTop: -15, fontFamily: fontStyle.FONT_FAMILY_MEDIUM, fontSize: RFValue(10, width)}}> BIO </Text>
                    </View>
                    ...
                    <Button full style={{height: 7*height/100, marginBottom: 3*height/100, borderRadius: 10, width: 85*width/100, alignSelf: 'center', opacity: this.state.buttonOpacity, backgroundColor: '#B34D8C', justifyContent: 'center'}} onPress={() => this.changeBIO()}>
